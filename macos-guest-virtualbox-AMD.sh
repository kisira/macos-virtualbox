...
function set_variables() {
    ...
    if [[ "$(sw_vers 2>/dev/null)" && "${get_parameters_from_macOS_host}" =~ [Yy] ]]; then
        ...
        model_name="${hardware_overview##*Model Name: }"
        model_name="${model_name%%$'\n'*}"
        model_identifier="${hardware_overview##*Model Identifier: }"
        model_identifier="${model_identifier%%$'\n'*}"
        boot_rom_ver="${hardware_overview##*Boot ROM Version: }"
        boot_rom_ver="${boot_rom_ver%%$'\n'*}"
        sn_system="${hardware_overview##*Serial Number (system): }"
        sn_system="${sn_system%%$'\n'*}"
        hardware_uuid="${hardware_overview##*Hardware UUID: }"
        hardware_uuid="${hardware_uuid%%$'\n'*}"
        nvram_rom="$(nvram 4D1EDE05-38C7-4A6A-9CC6-4BCCA8B38C14:ROM)"
        nvram_rom="${nvram_rom##*$'\t'}"
        nvram_mlb="$(nvram 4D1EDE05-38C7-4A6A-9CC6-4BCCA8B38C14:MLB)"
        nvram_mlb="${nvram_mlb##*$'\t'}"
        ioreg_board_id="$(ioreg -p "IODeviceTree" -r -n / -d 1)"
        ioreg_board_id="${ioreg_board_id##*board-id\" = <\"}"
        ioreg_board_id="${ioreg_board_id%%\">*}"
        ioreg_system_id="$(ioreg -p "IODeviceTree" -n platform -r)"
        ioreg_system_id="${ioreg_system_id##*system-id\" = <}"
        ioreg_system_id="${ioreg_system_id%%>*}"
...
function check_dependencies() {
    ...
    if [[ -z "$(gzip --help 2>/dev/null)" || -z \
    "$(unzip -hh 2>/dev/null)" || -z \
    "$(csplit --help 2>/dev/null)" || -z \
    "$(wget --version 2>/dev/null)" ]]; then
    ...
    if ! dmg2img >/dev/null 2>&1; then
        ...
        if [[ -z "$("${PWD%%/}/dmg2img.exe" -d 2>/dev/null)" ]]; then
            if [[ -z "${PWD}" ]]; then
                echo "PWD environment variable is not set. Exiting."
                exit
            fi
...
function populate_bootable_installer_virtual_disk() {
    ...
    if [[ -n $(
        VBoxManage 2>&1 storagectl "${vm_name}" --add sata --name SATA --hostiocache on >/dev/null
    ) ]]; then
        echo "Could not configure virtual machine storage controller. Exiting."
        exit
    fi
    if [[ -n $(
        VBoxManage 2>&1 storageattach "${vm_name}" --storagectl SATA --port 1 --hotpluggable on \
            --type hdd --nonrotational on --medium "${macOS_release_name}_bootable_installer.${storage_format}" >/dev/null
    ) ]]; then
        echo "Could not attach \"${macOS_release_name}_bootable_installer.${storage_format}\". Exiting."
        exit
    fi
    if [[ -n $(
        VBoxManage 2>&1 storageattach "${vm_name}" --storagectl SATA --port 2 \
            --type dvddrive --medium "${macOS_release_name}_installation_files.viso" >/dev/null
    ) ]]; then
        echo "Could not attach \"${macOS_release_name}_installation_files.viso\". Exiting."
        exit
    fi
    if [[ -n $(
        VBoxManage 2>&1 storageattach "${vm_name}" --storagectl SATA --port 3 --hotpluggable on \
            --type hdd --nonrotational on --medium "${macOS_release_name}_BaseSystem.${storage_format}" >/dev/null
    ) ]]; then
        echo "Could not attach \"${macOS_release_name}_BaseSystem.${storage_format}\". Exiting."
        exit
    fi
...
sed -i.bak2 -e "/InstallESD\.dmg/{n;N;N;N;d;}" "${install_path}InstallInfo.plist" && \' >>"${vm_name}_bootinst.txt"
    ...
    if [[ -n $(
        VBoxManage 2>&1 storageattach "${vm_name}" --storagectl SATA --port 4 \
            --type dvddrive --medium "${vm_name}_populate_bootable_installer_virtual_disk.viso" >/dev/null
    ) ]]; then
        echo "Could not attach \"${vm_name}_populate_bootable_installer_virtual_disk.viso\". Exiting."
        exit
    fi
...
be detached from the VM and released from VirtualBox."
    ...
    if [[ -n $(
        VBoxManage 2>&1 storageattach "${vm_name}" --storagectl SATA --port 3 --medium none >/dev/null
        VBoxManage 2>&1 closemedium "${macOS_release_name}_BaseSystem.${storage_format}" >/dev/null
    ) ]]; then
...
function populate_macos_target_disk() {
    ...
    if [[ -n $(
        VBoxManage 2>&1 storageattach "${vm_name}" --storagectl SATA --port 0 \
            --type hdd --nonrotational on --medium "${vm_name}.${storage_format}" >/dev/null
    ) ]]; then
        echo "Could not attach \"${vm_name}.${storage_format}\". Exiting."
        exit
    fi
    if [[ -n $(
        VBoxManage 2>&1 storageattach "${vm_name}" --storagectl SATA --port 1 --hotpluggable on \
...
./startosinstall --agreetolicense --pidtosignal ${background_pid} --rebootdelay 90 --volume "/Volumes/'"${vm_name}"'"' >>"${vm_name}_startosinstall.txt"
    if [[ -n $(
        VBoxManage 2>&1 storageattach "${vm_name}" --storagectl SATA --port 3 \
            --type dvddrive --medium "${vm_name}_populate_macos_target_disk.viso" >/dev/null
    ) ]]; then
        echo "Could not attach \"${vm_name}_populate_macos_target_disk.viso\". Exiting."
        exit
    fi
...
function prompt_delete_temporary_files() {
    ...
    if [[ ! "$(VBoxManage showvminfo "${vm_name}")" =~ State:[\ \t]*powered\ off ]]; then
...
function prompt_lang_utils_terminal() {
    ...
    if [[ "${tesseract_ocr}" =~ ${regex_ver} && "${tesseract_lang}" =~ eng ]]; then
        ...
        for i in $(# try automatic ocr for about 5 minutes
            seq 1 60
        ); do
